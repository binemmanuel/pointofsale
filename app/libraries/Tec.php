...
class Tec
{
    ...
    public function accumulatedBalance($id){
        if (empty($this->site->getTotalCustomerSales($id))) {
            return "0.00";
        }else{
            $custsales =  $this->site->getTotalCustomerSales($id);
            $salesbalance = $custsales->balance;
            $walletbalance = $this->site->getCustomerWalletBalance($id);
            return $walletbalance;
        }
    }

    public function getStore($id = NULL) {
        return $this->site->getStoreNameByID($id);
    }

     public function getUserName($id){
        return $this->site->getUserName($id);
    }
    ...
    // Send SMS
    public function sendsms($message, $sendto)
    {

        
        $smsapi = "https://sms.hollatags.com/api/send/?";
        $user;
        $username = $this->Settings->sms_username;
        $password = $this->Settings->sms_password;
        $senderid = $this->Settings->sms_senderid;
        $arr = [
            'user' => $username,
            'pass' => $password,
            'msg' => $message,

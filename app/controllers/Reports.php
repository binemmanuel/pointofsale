...
class Reports extends MY_Controller
{
    ...
    function daily_sales($year = NULL, $month = NULL) {
        ...
        if (!empty($sales)) {
            foreach ($sales as $sale) {
                ...
                "</td><td style='text-align:right;'>{$this->tec->formatMoney($sale->total)}</td></tr><tr><td><span style='font-weight:normal;'>".lang('product_tax')."<br>".'Service Charge'."</span><br>".lang('tax').
    ...
    function get_payments() {
        ...
        $this->load->library('datatables');
        $this->datatables
        ->select("{$this->db->dbprefix('payments')}.id as id, {$this->db->dbprefix('payments')}.date, {$this->db->dbprefix('payments')}.reference as ref, {$this->db->dbprefix('sales')}.id as sale_no, paid_by, amount, {$this->db->dbprefix('payments')}.pos_balance as balance")
        ->from('payments')

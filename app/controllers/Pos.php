...
class Pos extends MY_Controller {
    ...
    function index($sid = NULL, $eid = NULL) {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!$eid && !$suspend && $paid) {
                ...
                $balance_amount = $this->input->post('addtowallet') && $this->input->post('balance_amount') > 0 ? 0 : $this->input->post('balance_amount');
                $amount = $this->tec->formatDecimal(($paid > $grand_total ? ($paid - $balance_amount) : $paid), 4);
                $cwalletbalance = $this->site->getCustomerWalletBalance($customer_id);
                $amount = $this->input->post('amount');
                $paid_by = $this->input->post('paid_by');
                if($this->input->post('addtowallet') && $this->input->post('balance_amount') > 0) {
                    $change = $this->input->post('balance_amount');
                    $newbalance = $cwalletbalance + $change;
                    $this->site->updateWallet($customer_id,$newbalance);
                }
                //var_dump($paid_by); die();
                if($paid_by == "wallet"){
                    $newbalance = $cwalletbalance - $amount;
                    if ($cwalletbalance < 0) {
                        $this->session->set_flashdata('error','Cannot Pay With Negative Balance. Add credit to wallet');
                        redirect("pos");
                    }
                ...
                $payment = array(
                    ...
                    'pos_balance' => ($paid_by == "wallet") ? 0 : $this->tec->formatDecimal($balance_amount, 4)
                    );
    ...
    function addchangetowallet(){
        $change = $this->input->post('change');
        $customer = $this->input->post('customer');
        $saleid = $this->input->post('saleid');
        $cwalletbalance = $this->site->getCustomerWalletBalance($customer);
        $newbalance = $cwalletbalance + $change;
        $redirect_to = "pos/view/" . $saleid;
        if ($change && $this->site->updateWallet($customer,$newbalance)) {
            $this->site->baladdtowallet($saleid);
            $this->session->set_flashdata('message', 'Balance Added to Wallet Successfully');
            redirect($redirect_to);
        }else{
            $this->session->set_flashdata('error', 'Could Not');
            redirect($redirect_to);
        }

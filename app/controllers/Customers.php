...
class Customers extends MY_Controller
{
    ...
    function get_customers() {
        ...
        $this->datatables
        ->select("{$this->db->dbprefix('customers')}.id as id, {$this->db->dbprefix('customers')}.name as name, {$this->db->dbprefix('customers')}.phone as phone, {$this->db->dbprefix('customers')}.email as email, {$this->db->dbprefix('customers')}.address as address, {$this->db->dbprefix('customers')}.customerno as customerno, COALESCE(sum({$this->db->dbprefix('payments')}.pos_balance)) as balance, {$this->db->dbprefix('customers')}.walletbalance as walletbalance")
        ->from("customers")
        ->join('payments', 'payments.customer_id=customers.id', 'left')
    ...
    function add() {
        ...
        $this->form_validation->set_rules('customerno', 'customer number', 'required|is_unique[customers.customerno]');
    ...
    function walletbalance() {
        ...
        $response = $this->site->getCustomerWalletBalance($searchTerm);
        echo json_encode($response);
    }

    function customerDetailsJson() {
    
        $custid = $this->input->get('custid');
        $data['customer'] = $this->site->getCustomerByID($custid);
